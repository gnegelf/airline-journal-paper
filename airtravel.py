        ...
        for i,j in TRIP0:
          for p in PLANE:
            if value_of_y[i,j,p] > 0:
              ...
              for k in range(1,y_upper_bound[i,j,p]+1):
                thevars.append(eta[i,j,p,k])
                thecoefs.append(-1.0)
              therhs += 0.0
...
for p,i,j in TIMEFREEPLANESOLUTION:
    primal_objective += multiple_arc_use[p,i,j]*travelcost[i,j,p]
...
for r in REQUEST:
  for i,j in REQUEST_TRIP0[r]:
    for p in PLANE:
      ...
      model.variables.add(names = [x[i,j,r,p]], lb = [0], ub = [1], types = ["B"])
...
for i,j in TRIP0:
  for p in PLANE:
    ...
    model.variables.add(names = [y[i,j,p]], lb = [0], types = ["I"])
    number_of_variables += 1
...
# MAIN LOOP: forall <i,j,k> in TRIPS*PLANES
# - set objective function to max y[i,j,k]
# - solve this mip to time-free optimal solution
# - or: solve this mip to optimal solution with time
...
for i,j in TRIP0:
  for p in PLANE:
    print "maximizing trip (",i,",",j,"), plane ",p 
    
    # set new objective function
    model.objective.set_linear(y[i,j,p],1.0)
    ...
    if solution.is_primal_feasible():
      y_upper_bound[i,j,p] = int(round(solution.get_objective_value()))
      print "  upper bound on trips:",y_upper_bound[i,j,p]
    else:
      y_upper_bound[i,j,p] = -1
      print "  no solution exists:"
    
    # delete old objective function
    model.objective.set_linear(y[i,j,p],0.0)
    
    
# set new upper bounds to variables

for i,j in TRIP0:
  for p in PLANE:
     print "maximizing trip (",i,",",j,"), plane ",p,": ",y_upper_bound[i,j,p]
     model.variables.set_upper_bounds(y[i,j,p],y_upper_bound[i,j,p])
     
     
# set new objective function and sense

for r in REQUEST:
  for i,j in REQUEST_TRIP0[r]:
    for p in PLANE:
      model.objective.set_linear(x[i,j,r,p],0.0001)

for i,j in TRIP0:
  for p in PLANE:
    model.objective.set_linear(y[i,j,p],travelcost[i,j,p])
...
# change y[i,j,p] variables from integer to continuous
